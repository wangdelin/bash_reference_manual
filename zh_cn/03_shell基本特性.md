...
双引号内的字符串前面加上'$'标志，会导致该字符串根据当前的区域设置来翻译。如果当前的区域设置为C或者POSIX，那么'$'标志会被忽略。如果字符串被翻译和替代，该替代是双引号的。
...
shell函数是一种以单个名称来分组命令以供后续使用的方法。他们像普通命令一样执行。当shell函数的名称用作简单的命令名称时，执行与该函数名称相关联的命令列表。shell函数在当前的shell上下文中执行。不会创建新的进程。
...
这会定义一个名为name的shell函数。保留字function是可选的，括号也是可选的。函数体是复合命令compound-command（查看3.2.4 复合命令）。该命令通常是{和}之间的列表，但可能是上面列出的任何复合命令，除了一个例外。如果使用function保留字，但是没有提供()，则必须提供{}。只要name被指定为一个命令的名称，那么compound-command就会被执行。当shell在POSIX模式时（查看6.11 Bash POSIX模式），name不能与特殊的内置命令（查看4.5 特殊内置命令）一样。任何与shell函数关联的重定向（查看重定向）在函数执行的时候被执行。

函数定义可以通过使用`unset`内置命令（查看4.1 Bourne Shell内置命令）加-f选项删除。

函数定义的退出状态是0，除非发生了语法错误或者存在只读的同名函数。当函数执行时，函数的退出状态是函数体中最后执行命令的退出状态。

> **注意:**

> - 由于历史原因，在最常见的用法中，函数体前后的大括号必须使用空白或者换行与函数体分开。这是因为大括号是保留字，只有当它们与命令列表被空白或者其他shell元字符分开的时候，才能被识别。
> - 此外，当使用大括号时，列表必须以';', '&', 或者换行终止。

当函数被执行时，函数的参数在执行过程中成为位置参数（查看3.4.1 位置参数）。被扩展为位置参数个数的特殊参数'#'被更新以反映更改。特殊参数'0'不会改变。函数执行时，FUNCNAME变量的第一个元素被设置为该函数的名称。

shell执行环境的所有其他方面在函数与它的调用者中是相同的，除了两个例外：DEBUG和RETURN traps不被继承，除非使用declare内置函数给出了trace属性，或者使用`set`内置命令开启了-o functrace选项，（在这种情况下所有函数继承DEBUG和RETURN traps）。ERR trap不被继承，除非开启了-o errtrace shell选项。查看4.1 Bourne Shell 内置命令，了解更多关于`trap`内置命令的描述。

如果FUNCNEST变量被设置为大于0的值，则定义了最大的函数嵌套级别。超出限制的函数调用将导致整个命令中止。

如果函数中内置命令`return`执行，函数完成，并在函数调用后继续执行下一个命令。任何与RETURN trap关联的命令在恢复执行下一个命令之前执行。当函数完成时，位置参数和特殊参数"#"的值将恢复为函数执行前的值。如果传递一个数字参数给`return`命令，那么它就是函数的返回状态。否则，函数的返回状态是return命令前最后执行命令的退出状态。

可以使用`local`内置函数来声明函数的局部变量。这些变量只对函数及其调用的命令是可见的。

函数名和定义可以使用`declare`(`typeset`)内置命令（查看4.2 Bash内置命令）加-f选项列出来。`declare`(`typeset`)加-F选项仅列出函数名（如果开启了extdebug shell选项，可列出源文件和行号）。可以使用`export`内置命令（查看4.1 Bourne Shell内置命令）将函数导出，以便子shell自动拥有它们的定义。

> **注意:**

> - 有相关名称的函数和变量可能会导致环境中多个相同命名的实体传递给shell子节点。应该小心，这种情况下可能会导致问题发生。

函数可以递归。FUNCNEST变量可用于限制函数调用堆栈的深度并限制函数调用的数量。默认情况下，对递归调用次数没有限制。
...
扩展的顺序是: 大括号扩展；波浪号扩展，参数和变量扩展，算术扩展，命令替换（以从左到右的方式完成）；词分割；文件名扩展。

在支持的系统上，还有额外的扩展可用：进程替换。它是与波浪号扩展，参数和变量扩展，算术扩展，命令替换同时进行。

在这些扩展执行后，原始词中出现的引用字符将被删除，除非他们自己被引用（引用删除）。

仅大括号扩展，词分割，文件名扩展可以改变扩展的词的数量。其他扩展是将一个词扩展为另一个词。唯一的例外是"$@"扩展（查看3.4.2 特殊参数）和"${name[@]}"（查看6.7 数组）。

所有扩展后，引用删除（查看3.5.9 引用删除）被执行。

### 3.5.1 大括号扩展

大括号扩展是一个可以产生任意字符串的机制。这种机制类似于文件名扩展（查看3.5.8 文件名扩展），但生成的文件名不需要存在。要被大括号扩展的模式是以一个可选的前缀，后面是由一对大括号包裹的一系列由逗号分隔的字符串或一个序列表达式，后跟可选的后缀。前缀是打括号中每个字符串的前缀，然后后缀是附加给每个结果字符串的内容，扩展是从左到右的。

大括号可以嵌套。每个扩展字符串的结果未排序; 从左到右的顺序被保留。例如，
...
序列表达式采取`{x..y[..incr]}`这样的形式，x和y可以是整数也可以是单个字符，incr可选，它是一个整数。当x和y是整数时，表达式扩展为表x和y之间的每个数字，包括x和y。在提供的整数前加'0'可以强制每项具有相同的宽度。当x或y以'0'开始时，sshell尝试强制所有生成的项目包含相同个数的数字，必要时补0。当x和y是字符时，该表达式以字母顺序扩展x和y之间的每个字符，包含x和y，默认使用C语言环境。注意，x和y必须为相同类型。当提供incr时，它被用作每项之间的差值，默认增量为1或-1。

大括号扩展是在其他任何扩展之前执行的，其他扩展中特殊的字符都会在结果中保留。它是严格的原文。bash不会对扩展的上下文或者大括号中的文本应用任何语法解释。为避免与参数扩展的冲突，字符串'${'不被视为是合格的打括号表达式。

正确的形式的大括号扩展必须包含不带引号的开、闭大括号，以及至少有一个不带引号的逗号，或者一个有效的序列表达式。任何不正确形式的大括号扩展都会保持不变。

{或者','必须被反斜杆引用，以防止它被视为大括号表达式的一部分。为避免与参数扩展的冲突，字符串'${'不被视为是合格的打括号表达式。
...
如果一个词以未加引号的波浪号('~')开头，那么直到第一个未加引号的斜杆（或所有字符，如果没有非引号的斜杠）的所有字符被认为是波浪号前缀。如果波浪号前缀中没有字符被引用，波浪号前缀中跟在波浪号后面的字符被认为是可能的登录名。如果这个登录名是空字符，波浪号则会被shell变量HOME替代。如果HOME没有设置，则用执行shell的用户的主目录替代。否则，
波浪号前缀被替代为与指定登录名有关联的主目录替代。

如果波浪号前缀是'\~+'，那么它将被shell变量PWD的值替代。如果波浪号前缀是'~-'， 那么它将被shell变量OLDPWD的值替代（如果已设置）。

如果波浪号前缀中波浪号后面的字符包含数字N，以可选的'+'或'-'字符做前缀，那么该波浪号前缀会被目录栈中相应的元素替代，就犹如被`dirs`内置命令用波浪号前缀中波浪号后跟字符为参数调用显示的字符串（查看6.8 目录栈）。如果波浪号前缀中波浪号后由不带'+'或'-'前缀的数字组成，则假定为'+'。

如果登录名无效，或者波浪号扩展失败，则词保持不变。

每个变量赋值检查紧跟在':'或第一个'='后的未引用的波浪号前缀。在这种情况下，波浪号扩展也会被执行。
因此，可以在PATH, MAILPATH, 和CDPATH赋值中使用带波浪号的文件名，shell将使用扩展后的值。
...
shell脚本是包含shell命令的文本文件。当这样的文件被用作调用Bash时的第一个非选项参数时，并且不提供-c或者-s选项，Bash会从文件中读取并执行命令，然后退出。这种模式会创建一个非交互式的shell。shell会首先在当前文件夹下搜索文件，如果没有发现则在$PATH定义的文件夹下查找。

当Bash运行一个shell脚本时，它会设置特殊参数0为文件的名称，而不是shell的名称，并且如位置参数被设置为剩余的参数。如果没有提供额外参数，则位置参数未设置。

可通过使用`chmod`命令打开执行位，来使shell脚本可执行。当Bash在搜索$PATH中搜索命令时，发现了这样的文件，它会生成一个subshell来执行它。换句话说，执行
...
如果filename是一个可执行的shell脚本。这个subshell重新初始化自己，所以效果好象是调用一个新的shell来解释这个脚本，除了parent记录的命令位置（查看4.1 Bourne Shell内置命令中hash的描述）被child保留外。

大多数版本的Unix使它成为操作系统命令执行机制的一部分。如果脚本的第一行以'#!'这两个字符开头，该行余下的部分为程序指定解释器。因此，你可以指定Bash，awk，Perl或其他一些解释器，并使用该语言编写脚本文件的其余部分。

解释器的参数由跟在脚本文件第一行的解释器名后的单个可选参数组成，后跟脚本文件名称，后跟其余的参数。Bash将在操作系统上执行这个动作，操作系统自己不会处理它。请注意，一些较旧版本的Unix将解释器名称和参数限制为最多32个字符。

Bash脚本常以#! /bin/bash（假设Bash安装在/bin下），因为这样可以确保Bash将永远解释脚本，即使它在另一个shell下执行。